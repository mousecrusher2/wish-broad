...
Run `pnpm check` before opening a PR; it executes TypeScript, ESLint, and `pnpm check:unused-exports` via Knip. Use `pnpm format` to apply Prettier defaults repo-wide and `pnpm format:check` for formatting verification. Run `pnpm schema:check` after editing D1 migrations; it requires the `sqlite3` CLI on `PATH`, replays `migrations/*.sql` into a temporary SQLite database, and compares the resulting schema shape with authoritative `schema.sql`. This mitigates schema drift only; it cannot prove that data migration logic preserves data correctly. `pnpm build:analyze` emits bundle analysis artifacts, and `pnpm deploy` builds and publishes the Worker and static assets to Cloudflare.
