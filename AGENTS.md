...
Use `npm run dev` for the Vite client, `npm run dev:worker` for a local Worker-only session, and `npm run dev:full` when you need Wrangler to serve the integrated app. Run `npm run build` to type-check and bundle the frontend. Run `npm run check` before opening a PR; it executes TypeScript and ESLint. Use `npm run format` to apply Prettier defaults repo-wide. `npm run deploy` publishes the Worker and static assets to Cloudflare.
...
## Testing Guidelines
There is no committed automated test suite or coverage threshold yet. Treat `npm run check` as the minimum validation step, then manually verify login, `/api/me`, `/api/lives`, and publish/play flows in Wrangler. If you add tests, prefer `*.test.ts` or `*.test.tsx` near the feature they cover.
...
Keep secrets in `.dev.vars`, which is gitignored, and keep binding names aligned across `wrangler.jsonc`, `.dev.vars`, and `worker/types.ts`. After changing Worker bindings, run `npm run cf-typegen` to refresh generated Cloudflare types.
